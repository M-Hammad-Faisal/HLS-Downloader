...
async def download_hls(url, out_path, res_text, bw, headers, conc, remux):
    async with aiohttp.ClientSession() as session:
        ...
        if not variants:
            ...
            print(
                f"Selected variant: {chosen.resolution or 'unknown'} @ {chosen.bandwidth or 'unknown'} bps"
            )
...
def get_timeout():
    while True:
        timeout_input = input(
            "\nEnter timeout in seconds (press Enter for default 30): "
        ).strip()
...
def select_resolution_interactive(captured_items):
    ...
    for item in captured_items:
        ...
        if (
            ...
            or url_lower.endswith(".ts")
            or ".m3u8" in url_lower
            or "video" in content_type_lower
            or "application/vnd.apple.mpegurl" in content_type_lower
            or "application/x-mpegurl" in content_type_lower
        ):
    ...
    while True:
        try:
            choice = input(
                f"\nSelect media item to download (1-{len(media_items)}): "
            ).strip()
            if not choice:
                continue
            index = int(choice) - 1
...
def build_argparser():
    p = argparse.ArgumentParser(
        description="HLS Downloader - Download HLS streams and media files"
    )
    p.add_argument(
        "--url",
        help="Source URL (direct media or .m3u8) - if not provided, interactive mode will be used",
    )
    p.add_argument(
        "--interactive",
        "-i",
        action="store_true",
        help="Force interactive mode (capture from page)",
    )
    p.add_argument(
        "--out",
        default=str(Path.cwd() / "downloads" / "output.mp4"),
        help="Output file path",
    )
    p.add_argument(
        "--mode", choices=["auto", "http", "hls"], default="auto", help="Download mode"
    )
    ...
    p.add_argument(
        "--no-remux", action="store_true", help="Do not remux HLS to MP4 (keep .ts)"
    )
    p.add_argument(
        "--no-headless",
        action="store_true",
    ...
    if use_interactive:
        ...
        try:
            ...
            if not captured_items:
                print(
                    "No media items captured. Please check the page URL and try again."
                )
                sys.exit(1)
